...
// had to copy and override the whole thing to make sure our template-big-hero-homepage had the right attributes

// maybe we could have added a different box *only* in the case of our new template but some stuff would still be
// in the UW attributes and it feels weird to have that template be different than all the others
class HPRC_Page_Attributes_Meta_Box
{
  ...
  function replace_meta_box()
  {
      ...
      remove_meta_box( 'uwpageparentdiv', 'page', 'side');
	    add_meta_box( 'hprcpageparentdiv', 'Page Attributes', array( $this, 'page_attributes_meta_box' ), 'page', 'side', 'core' );
  }
  ...
  function page_template_dropdown( $default = '' , $post) {
    ...
    echo "</div>";
    if ($default === "templates/template-big-hero.php" || $default === "templates/template-small-hero.php" || $default === "templates/template-big-hero-homepage.php") {
...
new HPRC_Page_Attributes_Meta_Box;
